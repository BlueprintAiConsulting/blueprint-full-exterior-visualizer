    ...
    <div className="flex-1 relative bg-[#0A0E17] flex items-center justify-center overflow-hidden">
      <AnimatePresence mode="wait">
        {currentResult ? (
          ...
          >
            {/* Zoom & Compare mode toolbar */}
            <div className="absolute top-3 inset-x-4 sm:left-auto sm:right-4 flex flex-wrap items-center justify-between sm:justify-end gap-2 bg-[#0F172A]/95 backdrop-blur-md border border-[#334155] rounded-xl sm:rounded-full px-4 py-2 shadow-2xl z-30">
              {/* Compare Mode Segmented Control */}
              <div className="flex bg-[#0A0E17] p-0.5 rounded-lg border border-[#1E293B] mr-2">
                {[
                  { id: 'slider', label: 'Split Slider' },
                  { id: 'side-by-side', label: 'Side-by-Side' },
                  { id: 'toggle', label: 'Toggle/Hold' },
                ].map(mode => (
                  <button
                    key={mode.id}
                    onClick={() => {
                      setCompareMode(mode.id as any);
                      if (mode.id !== 'slider') setZoom(1); // reset zoom for better side-by-side fit
                    }}
            ...
            <div
              className={`relative w-full flex-1 min-h-0 rounded-lg overflow-hidden border border-[#334155] shadow-2xl bg-[#0F172A] select-none ${isPanMode ? (isDraggingPan ? 'cursor-grabbing' : 'cursor-grab') : compareMode === 'slider' ? 'cursor-ew-resize' : 'cursor-default'}`}
              onWheel={(e) => {
              ...
              onMouseDown={(e) => {
                if (compareMode !== 'slider') {
                  if (isPanMode) onStartPan(e.clientX, e.clientY);
                  return;
                }
                if (isPanMode) {
            ...
            >
              ...
              {compareMode === 'side-by-side' && (
                <div className="absolute inset-0 flex flex-col md:flex-row gap-2 p-2 pt-14 sm:pt-4">
                  <div className="flex-1 relative rounded-lg border border-[#1E293B] overflow-hidden bg-[#0A0E17] flex items-center justify-center">
                    <img src={selectedImage!} alt="Before" className="max-w-full max-h-full object-contain pointer-events-none" />
                    <div className="absolute bottom-3 left-3 bg-[#0F172A]/90 backdrop-blur-sm text-[#94A3B8] text-[9px] font-bold uppercase tracking-[0.2em] px-3 py-1.5 rounded border border-[#334155] pointer-events-none">BEFORE</div>
                  </div>
                  <div className="flex-1 relative rounded-lg border border-[#1E293B] overflow-hidden bg-[#0A0E17] flex items-center justify-center">
                    <img src={currentResult} alt="After" className="max-w-full max-h-full object-contain pointer-events-none" />
                    <div className="absolute bottom-3 left-3 bg-[#0EA5E9]/90 backdrop-blur-sm text-white text-[9px] font-bold uppercase tracking-[0.2em] px-3 py-1.5 rounded border border-white/20 pointer-events-none">AFTER</div>
                  </div>
                </div>
              )}
              ...
              {compareMode === 'toggle' && (
                <>
                  ...
                  >
                    <img src={showBeforeToggle ? selectedImage! : currentResult} alt="Comparison" className="absolute inset-0 w-full h-full object-contain pointer-events-none" draggable={false} />
                  </div>
                  <div className="absolute top-4 left-4 bg-[#0F172A]/85 backdrop-blur-md text-[#94A3B8] text-[9px] font-bold uppercase tracking-[0.2em] px-3 py-1.5 rounded-sm shadow-lg border border-[#334155] pointer-events-none z-10">
                    {showBeforeToggle ? 'BEFORE' : 'AFTER'}
                  </div>
                  
                  {/* Hold to View Button */}
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 z-20">
                    <button 
                      onMouseDown={() => setShowBeforeToggle(true)}
                      onMouseUp={() => setShowBeforeToggle(false)}
                      onMouseLeave={() => setShowBeforeToggle(false)}
                      onTouchStart={(e) => { e.preventDefault(); setShowBeforeToggle(true); }}
                      onTouchEnd={() => setShowBeforeToggle(false)}
                      className="px-6 py-3 bg-[#1D4ED8] hover:bg-[#1E3A8A] text-white text-xs font-bold uppercase tracking-widest rounded-full border border-white/20 shadow-[0_0_24px_rgba(59,130,246,0.4)] active:scale-95 transition-all select-none cursor-pointer"
                    >
                      Hold to View Before
                    </button>
                    <span className="text-[8px] text-[#475569] font-medium tracking-wide">Press and hold to see original</span>
                  </div>
                </>
              )}
            </div>
